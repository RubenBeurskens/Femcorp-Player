...
local diskStorage
local lastSlot = settings.get("player.lastSlot",1)

local function findInventories()
  local allPeripherals = peripheral.getNames()
  for i,p in pairs(allPeripherals) do
    local methods = peripheral.getMethods(p)
    for i,o in pairs(methods) do
      if o == "pushItems" then
        diskStorage = peripheral.wrap(p)
        return
      end
    end
  end
...
local function mixToMono(left,right)
  local monoTable = {}
  for i,o in pairs(left) do -- loop through all samples
    if left and right and left[i] and right[i] then -- the result of femcorp audio player level of hopes and dreams bondage
      local monoSample = (left[i]/2) + (right[i]/2) --Divide each sample to mix, not the mix itself
      table.insert(monoTable,monoSample)
    end
  end
  return monoTable
end
    ...
    if disk.isDiskPresent() then
      ...
      elseif fs.isDir("disk/") and playing then
        ...
        if type == "mdfpwm" then
          ...
          swapDisk = false
          newDecoder()
          local monoL,monoR=dfpwm.make_decoder(),dfpwm.make_decoder()
          while true do
            while playing do
              bufferLeft = decoderLeft(song.getSample(seconds).left)
              bufferRight = decoderRight(song.getSample(seconds).right)
              local bufferMono = mixToMono(monoL(song.getSample(seconds).left),monoR(song.getSample(seconds).right))
              ...
              if modem then
                modem.transmit(50001,50001,{id = os.getComputerID(),bufferMono = bufferMono,bufferLeft = bufferLeft, bufferRight = bufferRight, swapDisk = swapDisk, title = title, artist = artist, album = album, seconds = seconds,runtime = runtime, volume = volume, playing = playing, id = os.getComputerID(), computationalVolume = not bitExact})
              end
        ...
        if diskStorage then
          error("switching disk")
          local pullAmount = diskStorage.pullItems(peripheral.getName(disk),1,_,lastSlot)
          if pullAmount == 0 then
            error("didnt push")
          end
          lastSlot = lastSlot + 1
          local count = diskStorage.pushItems(peripheral.getName(disk),lastSlot)
          if count == 0 then
            lastSlot = 1
            diskStorage.pushItems(peripheral.getName(disk),lastSlot)
          end
          settings.set("player.lastSlot",lastSlot)
          settings.save()
        end
      end
    else
...
local function drawDisplay()
  while true do
    ...
    -- time bar
    local totalLength = xSize-20 --30
    local playedLength = totalLength * (seconds / runtime)
    -- time bar
    for i=1,totalLength do
      local space = true
      term.setTextColor(colors.black)
      term.setCursorPos(xSize / 2 - totalLength / 2 + i, ySize/2+3)
      if i > playedLength then
        term.setBackgroundColor(colors.gray)
      elseif i==playedLength then
    ...
    term.setCursorPos(xSize / 2 - totalLength / 2 + 2,ySize/2+4)
    term.write(playedMinutes .. ":" .. string.format("%02d",seconds - playedMinutes * 60))
    term.setCursorPos(xSize / 2 - totalLength / 2 + totalLength - 4,ySize/2+4)
    ...
    term.setCursorPos(rewindPos,scaledButtonY-1)
    term.setBackgroundColor(colors.blue)
    term.write("    ")
    term.setCursorPos(rewindPos,scaledButtonY)
    term.write(" \17\17 ")
    term.setCursorPos(rewindPos,scaledButtonY+1)
    term.write("    ")
    ...
    term.setCursorPos(pausePos,scaledButtonY-1)
    term.setBackgroundColor(colors.blue)
    term.write("    ")
    term.setCursorPos(pausePos,scaledButtonY)
    term.write(" || ")
    term.setCursorPos(pausePos,scaledButtonY+1)
    term.write("    ")
    ...
    term.setCursorPos(playPos,scaledButtonY-1)
    term.setBackgroundColor(colors.blue)
    term.write("    ")
    term.setCursorPos(playPos,scaledButtonY)
    term.write(" \138\16 ")
    term.setCursorPos(playPos,scaledButtonY+1)
    term.write("    ")
    ...
    term.setCursorPos(stopPos,scaledButtonY-1)
    term.setBackgroundColor(colors.blue)
    term.write("    ")
    term.setCursorPos(stopPos,scaledButtonY)
    term.write(" \138\133 ")
    term.setCursorPos(stopPos,scaledButtonY+1)
    term.write("    ")
    ...
    term.setCursorPos(forwardPos,scaledButtonY-1)
    term.setBackgroundColor(colors.blue)
    term.write("    ")
    term.setCursorPos(forwardPos,scaledButtonY)
    term.write(" \16\16 ")
    term.setCursorPos(forwardPos,scaledButtonY+1)
    term.write("    ")
...
local function statusSender()
  while true do
    if modem then
      modem.transmit(50000,50000,{theme = settings.get("player.theme"), swapDisk = swapDisk, title = title, artist = artist, album = album, seconds = seconds,runtime = runtime, volume = volume, playing = playing, id = os.getComputerID(), computationalVolume = not bitExact})
    end
    sleep(0.5)
  end
end
...
local function inputsReceiver()
  while true do
    if not modem then
      modems = { peripheral.find("modem", function(name, modem) return modem.isWireless() end) }
      modem = modems[1]
    end
    if modem then
      ...
      sleep(1) --Sleep for 1 cycle, wait until a modem is added
    end
  end
end
